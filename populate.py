...
def populate_test_accounts():
    ...
    from accounts.models import BProfile
    from accounts.forms import BusinessProfileForm
    ...
    for idu, user_name in enumerate(usernames):
        ...
        if not user:
            ...
            user = user[0]
        Profile.objects.filter(user=user).update(business_account=business[idu], verified=True)
        if business[idu]:
            bdict = {
                "user": user
            }
            ...
            if bzform.is_valid():
                bzform.save()
                print(user.username, "BProfile created")
